...
\paragraph{Introdução do \textit{Scrambler}:} \hspace{0pt}

\todo{guilherme}

\paragraph{Implementação de NCO controlado pelo erro:} \hspace{0pt}
...
\paragraph{Implementação dos Filtros:} \hspace{0pt}

\todo{guilherme}

\paragraph{\textit{Loop} desmodulador:} \hspace{0pt}
 \todo{apenas um rascunho, falta meter as equações e rever a linguagem}
 
%misturado?
Nesta fase do projeto têm-se todos os blocos necessários do modem BPSK. O sinal de informação $b_n$ foi misturado, $e_n$, codificado, mapeado e posteriormente multiplicado à portadora com frequência $f_0 = 4kHz$, obtendo o sinal modulado BPSK,$s_n$. Este após a transmissão pode sofrer variações de fase ($\Delta\phi=\phi_0-\phi_1$) e de frequência ($\Delta f = f_1-f_0$) tomando a forma 
...
\paragraph{Testes do \textit{Costas Loop} com onda sinusoidal} \hspace{0pt}
...
\paragraph{Testes do \textit{Costas Loop} com BPSK sem zero}
...
\paragraph{Testes do \textit{Costas Loop} com BPSK com zero} \hspace{0pt}
...
\paragraph{Transient} \hspace{0pt}
