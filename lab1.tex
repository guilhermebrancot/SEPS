...
\begin{titlepage}
	\begin{center}
		...
		\footnote{As linhas de código apresentadas durante o relatório têm como objectivo demonstrar a maneira de raciocinar na resolução de problemas, não representando cuma cópia exacta do código usado em laboratório, podendo até, serem consideradas \textit{pseudo-código}}
		\vfill
...
\begin{table}[H]
	...
	\begin{tabular}[c]{|l||c|}
		\hline \textbf{Frequência de saída} & \textbf{Incremento}\\ 
...
A primeira variàvel foi já antes referida, nas secções \ref{para:P1-1} e \ref{para:P1-3}. Esta variàvel, caso alterada iria alterar a frequência da rampa, e por consequência, a frequência do sinal de saída. Como se pode observar na tabela \ref{tab:incrementos}, esta variàvel terá como limite máximo o valor $ 24576 $ e como limite mínimo $ 8192 $. Podemos também concluir que quanto maior for esta variàvel, maior a frequência do sinal de saída, tal como se o seu valor diminuir, a frequência de saía irá diminuir também.

A segunda variàvel (\textit{amplitude}) foi criada com o propósito de modelar a amplitude do sinal de saída, esta causou mudanças mais significativas no código, tal como se pode verificar:
...
Um exemplo de garantir que isto era possível era receber um novo valor para a variável \textit{incremento}, sendo que este valor vinha directamente da entrada:
...
Ambos os sinais podem ser observados na imagem \ref{fig:interp}, embora a sua comparação não poderá ser efectuada na sua observação.
...
\begin{figure}[H]
	...
	\caption{Sinal sinusóide com interpolação e sem interpolação.}
\end{figure}
...
Na imagem o espectro da quadrada centrada em 4khz como se esperava. As harmonicas estao espaçadas 500Hz pois o sinal $d_n$ consiste numa onda quadrada de frequência fundamental 250 Hz.
